...
<BODY>
	...
	<section class="flex1 dashboardSection db__header-info">
		...
		<div class="buttonsDiv">
			<a href="db-comments.php" class="db-buttons-current">Comments</a>
	...
	<section id="main-content" class="flex1">
		<div id="guts" class="form-group">
			...
			<?php
				...
				$sql = "SELECT id, name, email, comment, date, image FROM comments";
				$result = $conn->query($sql);
				while($comments = $result->fetch_assoc()) {
					echo "
						<tr>
							<td>
								<input type='text' name='id' value='".$comments['id']."' readonly size='2'>
							</td>
							<td>
								<input type='text' name='name' value='".$comments['name']."'>
							</td>
							<td>
								<input type='text' name='email' value='".$comments['email']."'>
							</td>
							<td>
								<input type='text' name='comment' value='".$comments['comment']."'>
							</td>
							<td>
								<input type='text' name='date' value='".$comments['date']."'>
							</td>
							<td>
								<input type='text' name='image' value='".$comments['image']."'>
							</td>
							<td>
								<input type='submit' class='edit' name='alter' value='edit_comment'>
							</td>
							<td>
								<input type='submit' class='delete' name='alter' value='delete_comment'>
							</td>
						</tr>
						";
				}		
		...
		<form action="alterTables" method="get">
			<table>
				...
				<tr>
					...
					<td>
						<input type="submit" class="add" name="alter" value="add_comment">
					</td>
				</tr>
			</table>
		</form>
