...
<BODY>
	...
	<section class="flex1 dashboardSection db__header-info">
		...
		<div class="buttonsDiv">
			...
			<a href="db-exhibitions.php" class="db-buttons-current">Exhibitions</a>
	...
	<section id="main-content" class="flex1">
		<div id="guts" class="form-group">
			...
			<?php
				...
				$sql = "SELECT id, name, start_date, end_date, description, image, link FROM exhibitions";
				$result = $conn->query($sql);
				while($exhib = $result->fetch_assoc()) {
					echo "
						<tr>
							<td>
								<input type='text' name='id' value='".$exhib['id']."' readonly size='2'>
							</td>
							<td>
								<input type='text' name='name' value='".$exhib['name']."'>
							</td>
							<td>
								<input type='text' name='start_date' value='".$exhib['start_date']."'>
							</td>
							<td>
								<input type='text' name='end_date' value='".$exhib['end_date']."'>
							</td>
							<td>
								<input type='text' name='description' value='".$exhib['description']."'>
							</td>
							<td>
								<input type='text' name='image' value='".$exhib['image']."'>
							</td>
							<td>
								<input type='text' name='link' value='".$exhib['link']."'>
							</td>
							<td>
								<input type='submit' class='edit' name='alter' value='edit_exhib'>
							</td>
							<td>
								<input type='submit' class='delete' name='alter' value='delete_exhib'>
							</td>
						</tr>
						";
				}		
		...
		<form action="alterTables" method="get">
			<table>
				...
				<tr>
					...
					<td>
						<input type="submit" class="add" name="alter" value="add_exhib">
					</td>
				</tr>
			</table>
			</form>
