...
<BODY>
	...
	<section class="flex1 dashboardSection db__header-info">
		...
		<div class="buttonsDiv">
			...
			<a href="db-reviews.php" class="db-buttons-current">Reviews</a>
	...
	<section id="main-content" class="flex1">
		<div id="guts" class="form-group">
			...
			<?php
				...
				$sql = "SELECT id, name, photo, review FROM reviews";
				$result = $conn->query($sql);
				while($reviews = $result->fetch_assoc()) {
					echo "
						<tr>
							<td>
								<input type='text' name='id' value='".$reviews['id']."' readonly size='2'>
							</td>
							<td>
								<input type='text' name='name' value='".$reviews['name']."'>
							</td>
							<td>
								<input type='text' name='photo' value='".$reviews['photo']."'>
							</td>
							<td>
								<input type='text' name='review' value='".$reviews['review']."'>
							</td>
							<td>
								<input type='submit' class='edit' name='alter' value='edit_review'>
							</td>
							<td>
								<input type='submit' class='delete' name='alter' value='delete_review'>
							</td>
						</tr>
						";
				}		
		...
		<form action="alterTables" method="get">
			<table>
				...
				<tr>
					...
					<td>
						<input type="text" name="addphoto">
					</td>
					<td>
						<input type="text" name="addreview">
					</td>
					<td>
						<input type="submit" class="add" name="alter" value="add_review">
					</td>
				</tr>
			</table>
		</form>
